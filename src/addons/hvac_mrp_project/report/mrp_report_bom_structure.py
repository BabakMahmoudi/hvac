...
if TYPE_CHECKING:
    ...
    HvacMrpProject, HvacProductExtensions, HvacProductTemplateExtensions, HvacUtils, HvacMrpBomExtensions, HvacMrpBomLineExtensions
else:
    HvacBom = models.Model
...
class ReportHvacBomStructure(models.AbstractModel):
    _name = 'report.mrp.hvac.report_bom_structure'
...
class ReportHvacBomExtensions(models.AbstractModel):
    ...
    @api.model
    def get_bom(self, bom_id=False, product_id=False, line_qty=False, line_id=False, level=False):
        return super().get_bom(bom_id, product_id, line_qty, line_id, level)
    def get_project(self)->HvacMrpProject:
        result = False
        model = self.env.context.get("model", False)
        if model == 'hvac.mrp.project':
            project_id = self.env.context.get("active_id", False)
            if project_id:
                result = self.env['hvac.mrp.project'].browse(project_id).exists()
        return result
    ...
    def get_html(self, bom_id=False, searchQty=1, searchVariant=False):
        ...
        if project:
            ...
            return super().get_html(bom.id, searchQty, searchVariant)
    ...
    def _get_report_data(self, bom_id, searchQty=0, searchVariant=False):
        ...
        # This is a project
        # we will loop true all boms

        all =[]
        components=[]
        result = False
        lines = {'babak':'babak'}
        for _line in project.sale_order_lines:
            bom_id = _line.bom_id
            if not bom_id:
                bom_id = _line.prod_id.bom_ids
            report_data = super()._get_report_data(bom_id.id, searchQty, searchVariant)
            all.append(report_data)
            #lines = report_data.lines
            component = {'babak':'babak'}
            component['prod_id']=bom_id.product_id
            component['prod_name'] = bom_id.product_id.display_name
            component['prod_qty'] = 1
            #'prod_uom': line.product_uom_id.name,
            component['prod_uom'] ='Unit'
            component['parent_id'] = bom_id
            #'total': sub_total,
            component['total'] = 1
            #     'attachments': self.env['mrp.document'].search(['|', '&',
            #         ('res_model', '=', 'product.product'), ('res_id', '=', line.product_id.id), '&', ('res_model', '=', 'product.template'), ('res_id', '=', line.product_id.product_tmpl_id.id)]),
            component['attachments'] =  self.env['mrp.document'].search([('res_model','=', 'pppppp')])

            #     'child_bom': line.child_bom_id.id,
            component['child_bom']= False
            components.append(component)
        lines = {'babak':'babak'}
        lines['bom'] = ''
        lines['product'] = 'product'
        lines['bom_qty'] = 1
        lines['bom_prod_name'] = 'bom_prod_name'
        lines['code'] = 'code'
        lines['components'] = components
        lines['total'] = 0
        ...
        result = \
            {
                ...
                'variants': "bom_product_variants",
                'bom_uom_name': "",
                'bom_qty':1 , ##  bom_quantity,
                'is_variant_applied': True, # self.env.user.user_has_groups('product.group_product_variant') and len(bom_product_variants) > 1,
                'is_uom_applied': True , # self.env.user.user_has_groups('uom.group_uom')
            }            
        return result
...
#         lines['total'] += total
        ...
        bom_id = project.sale_order_lines[0].product_id.bom_ids[0]
        return super()._get_report_data(bom_id.id,searchQty,searchVariant)
        lines = {}
        bom = self.env['mrp.bom'].browse(bom_id)
        for bom in project.bom_ids:
            bom_quantity = searchQty or bom.product_qty
            bom_product_variants = {}
            bom_uom_name = ''
            if bom:
